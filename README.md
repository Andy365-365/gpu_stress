...
按 `Ctrl+C` 停止，分两个阶段：
1. **第 1 次 Ctrl+C**：停止压测，进入**冷却监控**（温度/风扇/功耗回落曲线逐秒记录进日志）
2. **第 2 次 Ctrl+C**（或冷却满 **30 分钟**自动退出）：结束程序

## 输出

**屏幕**（主线程每 2 秒左右一行，压测阶段；冷却阶段逐秒一行）：

```
压测目标 : Physical Slot 2 (nvidia-smi GPU 0) — NVIDIA GeForce RTX 3090（总显存 23.6 GB）
硬件标识 : UUID GPU-5ade556b-b128-4dc7-53a8-81d6e5090d12
矩阵规模 : 4096 x 4096 x 4096 (FP32)，单矩阵 64.0 MiB
显存占用 : 约 192 MiB（3 块矩阵，停止后立即释放）
采样间隔 : 1000 ms（后台线程，独立于计算）
日志文件 : /data/gpu_stress/stress_gpu2_20260906_105834.log
开始压测… 按 Ctrl+C 停止（进入冷却监控，再按 Ctrl+C 退出）
  [GPU 2] 温度  89C  风扇 100%  利用率 100%  功耗  273 W  显存 23700 MiB  降频:是  |   21.44 TFLOPS  |  16 s
压测停止。累计 4400 次 GEMM，运行 47 秒，平均 12.88 TFLOPS。
进入冷却监控（最长 30 分钟自动退出，再按 Ctrl+C 立即退出）
  [冷却  2 s] 温度  84C  风扇 100%  利用率   0%  功耗  191 W  显存 23700 MiB  降频:是
  [冷却  5 s] 温度  82C  风扇 100%  利用率   0%  功耗  161 W  显存 23700 MiB  降频:否
冷却监控结束（手动退出，5 秒）。
已停止。累计 4400 次 GEMM，运行 47 秒，平均 12.88 TFLOPS。显存已释放。
```
...
**日志文件**（内容 = 终端输出内容，每行前加时间戳，含压测 + 冷却监控全过程，用于后期分析）：

```
[2026-09-06 10:59:19]   [GPU 2] 温度  88C  风扇  99%  利用率 100%  功耗  275 W  显存 23700 MiB  降频:是  |    9.26 TFLOPS  |  44 s
[2026-09-06 10:59:21] 压测停止。累计 4400 次 GEMM，运行 47 秒，平均 12.88 TFLOPS。
[2026-09-06 10:59:21] 进入冷却监控（最长 30 分钟自动退出，再按 Ctrl+C 立即退出）
[2026-09-06 10:59:22]   [冷却  1 s] 温度  85C  风扇 100%  利用率   0%  功耗  244 W  显存 23700 MiB  降频:是
[2026-09-06 10:59:26]   [冷却  5 s] 温度  82C  风扇 100%  利用率   0%  功耗  161 W  显存 23700 MiB  降频:否
[2026-09-06 10:59:26] 冷却监控结束（手动退出，5 秒）。
[2026-09-06 10:59:27] 已停止。累计 4400 次 GEMM，运行 47 秒，平均 12.88 TFLOPS。显存已释放。
```

路径：`/data/gpu_stress/stress_gpu<槽位>_<日期>_<时间>.log`，每行写完立即 flush（中途停止也保留已有数据）。
冷却监控数据可用于分析：温度回落曲线、风扇降速策略（何时从 100% 开始回落）、热降频解除时间点（降频:是→否）。
